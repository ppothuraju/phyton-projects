...
Let's add some test data (all these numbers are valid - you can invalidate them by changing any character).
    ...
    British: GB72 HBZU 7006 7212 1253 00 [length = 22]
    French: FR76 30003 03620 00020216907 50 [length = 27]
    German: DE02100100100152517108 [length = 22]
    Norway: NO08 GNT4 1427 092 [length = 15]

If you are an EU resident, you can use you own account number for tests.
'''
# IBAN Validator

#iban = "DE02100100100152517108"#input("Enter IBAN, please: ")
def ibanvalidtion(iban):
    iban = iban.replace(' ','')
    country_dict = {"GB":22,"FR":27,"DE":22,"NO":15}
    if not iban.isalnum():
        print(iban," - You have entered invalid characters.")
    elif len(iban) < 15:
        print(iban," - IBAN entered is too short.")
    elif len(iban) > 31:
        print(iban," - IBAN entered is too long.")
    else:
        if ((iban[:2],len(iban)) not in country_dict.items()): #if iban[:2] in country_dict and country_dict [iban[:2]] != len(iban)
            #print(country_dict.items())
            print(iban," - IBAN length invalid for the country.")
        else:      
            ...
            ibann = int(iban2)
            if ibann % 97 == 1:
                print(iban, " - IBAN entered is valid.")
            else:
                print(iban, " - IBAN entered is invalid.")

ibanvalidtion("GB72 HBZU 7006 7212 1253 00")
ibanvalidtion("FR76 30003 03620 00020216907 50")
ibanvalidtion("DE02100100100152517108")
ibanvalidtion("NO08 GNT4 1427 092")
ibanvalidtion("GB72 HBZU 7006 7212 1253 001")
ibanvalidtion("FR76 30003 03620 00020216907 5000000000")
ibanvalidtion("NO08 GNT")
ibanvalidtion("GB72-HBZU-7006-7212-1253-00")
